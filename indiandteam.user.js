// ==UserScript==
// @name         Meilong Scissors Counter (Indi + Team Photos)
// @namespace    sportsinfocus-meilong-scissors
// @version      4.0
// @description  Highlights, counts, auto-selects, and clears selection of individual and team photos with the "Clip" scissors icon on the Sports In Focus portal gallery. Draggable panel with emoji icon and refined banana-yellow design.
...
(function () {
  ...
  function runCount(mode, autoSelect) {
    ...
    const outlineColor = mode === 'team' ? '3px solid #2563eb' : '3px solid red';
    ...
    console.log(`${mode === 'team' ? 'Team' : 'Individual'} photos with clipped (Meilong Scissors) icon:`, matches.length);
  ...
  function addButton() {
    ...
    style.textContent = `
      ...
      #gsc-run-btn:active, #gsc-run-team-btn:active {
      ...
      #gsc-run-team-btn:hover {
        background: linear-gradient(135deg, #7db4fb, #1d4fd0);
      ...
      #gsc-count-row.gsc-team #gsc-count-label {
        color: #2447a3;
      }
      #gsc-count-value {
        font-size: 24px;
      ...
      #gsc-count-row.gsc-team #gsc-count-value {
        color: #1d3f9c;
      }
      #gsc-names {
        max-height: 240px;
    ...
    panel.innerHTML = `
      ...
      <div id="gsc-body">
        <button id="gsc-run-btn">✂️ Select Indi Clipped</button>
        <button id="gsc-run-team-btn">✂️ Select Team Clipped</button>
    ...
    function renderResults(mode, result) {
      countRow.classList.toggle('gsc-team', mode === 'team');
      countValue.textContent = result.count;
      namesContainer.innerHTML = result.names
        .map(n => `<div class="gsc-name-item${mode === 'team' ? ' gsc-team' : ''}">${n}</div>`)
        .join('');
    ...
    runBtn.addEventListener('click', () => {
      const result = runCount('individual', true);
      renderResults('individual', result);
    });

    // --- Team run button ---
    const runTeamBtn = panel.querySelector('#gsc-run-team-btn');
    runTeamBtn.addEventListener('click', () => {
      const result = runCount('team', true);
      renderResults('team', result);
    });
